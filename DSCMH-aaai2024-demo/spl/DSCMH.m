function [ImgToTxt,TxtToImg] = DSCMH(trainLabel, param, dataset)
seed = 2023;
% rng('default');
rng(seed);
cmm=15;%16;
...
%       SPLbXT1=bsxfun(@times,R1_SPLb,X1'); 
%       ZU1 = (SPLsXT1'*SPLbXT1');
%       U1 = myOrth(ZU1)';
      SPLsXT2=bsxfun(@times,R2_SPLs,V2'); 
%       SPLbXT2=bsxfun(@times,R2_SPLb,X2'); 
%       ZU2 = (SPLsXT2'*SPLbXT2');
%       U2 = myOrth(ZU2)';     
      U1=(X1'*SPLsXT1)/(V1*SPLsXT1);
      U2=(X2'*SPLsXT2)/(V2*SPLsXT2);
      
    % Vt-Step
    SPLsXB1=bsxfun(@times,R1_SPLs,X1); 
    SPLbXU1=bsxfun(@times,R1_SPLb,U1); 
    ZV1 = (SPLsXB1*SPLbXU1) + bit * databaseL * (databaseL' *  alpha*B');
    V1 = myOrth(ZV1)';
    SPLsXB2=bsxfun(@times,R2_SPLs,X2); 
    SPLbXU2=bsxfun(@times,R2_SPLb,U2); 
    ZV2 = (SPLsXB2*SPLbXU2) + bit * databaseL * (databaseL' *  beta*B');
    V2 = myOrth(ZV2)';
...
%      gamma1_SPLs=gamma1_SPLs*pace_rate1;
    ...
    LOSS2_s = LOSS2_s+bsxfun(@times,R2_SPLb,X2'-U2*V2);
%     LOSS2_s=X2'-U2*V2;
...
%    loss_each_text(epoch) = sum(L2_SPLs) ;
    ...
    gamma2_SPLs=pace*gamma2_SPLs;   
    SPL2_s=[SPL2_s,R2_SPLs];
...
%      gamma1_SPLb=gamma1_SPLb*pace_rate2;
    ...
    LOSS2_b = zeros(size(X2'));
    LOSS2_b = LOSS2_b+bsxfun(@times,X2'-U2*V2,R2_SPLs');
...
%      gamma2_SPLb=gamma2_SPLb*pace_rate2;
end
