function [ImgToTxt,TxtToImg] = DSCMH_iter(trainLabel, param, dataset)
seed = 2023;
% rng('default');
rng(seed);
cmm=16;
...
ImgToTxt(epoch)=out.MAP_test(1)
TxtToImg(epoch)=out.MAP_test(2) 
    % Ut-Step
    ...
    % rs^t-Step
    LOSS1_s = bsxfun(@times,R1_SPLb,X1'-U1*V1);%LOSS1_s+
...
%         SPL1_b=[SPL1_b,R1_SPLb];
    ...
    L2_SPLb = mapminmax(L2_SPLb, 0, cmm);    
...
%        SPL2_b=[SPL2_b,R2_SPLb];
 ...
 LapAold=B;
 obj(epoch) = norm(LapA-LapAold,'fro')^2/norm(LapAold,'fro')^2
        

end
save('DSCMH_30_flickr_64.mat','ImgToTxt','TxtToImg','obj');
